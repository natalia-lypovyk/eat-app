...
export class AuthenticationGuard implements CanActivate {
  ...
  private async handleAuthorization(request: {
    ...
    try {
      const headerCredentials = AuthenticationGuard.parseAuthorizationHeader(
        request.headers.authorization,
      );

      const credentials = AuthenticationGuard.decodeCredentials(
        headerCredentials,
      );
